...
if __name__ == '__main__':
    ...
    for size in [3]:
        ...
        for i in range(6, 10):
            ...
            agent = ValueIterationAgent(env, gamma=(i + 0.5)/10)
            vals, k = agent.value_iteration(theta)
            policy = agent.extract_policy()
            end = time.time()
        ...
        gamma_arr = [0.65, 0.75, 0.85, 0.95]
        best_vals = [[0.18765033, 0.3110799, 0.51207354, 0.21126403, 0.37879276,
                                             0.8819396, 0.03290349, 0., 0.],
                    [0.31209726, 0.44177339, 0.62175011, 0.31567922, 0.4751232,
                           0.90337934, 0.09660179, 0., 0.],
                    [0.49501917, 0.60891177, 0.74653074, 0.4581614, 0.5830804,
                           0.9284733, 0.2311282, 0., 0.],
                    [0.78403474, 0.84089695, 0.90121119, 0.7424685, 0.80152427,
                           0.96810291, 0.51291657, 0., 0.]]
        time_array = [0.009800195693969727, 0.009859085083007812, 0.009998083114624023,
                     0.01096487045288086]
        iters = [15, 13, 14, 23]
        list_scores = [0.92, 0.922, 0.952, 1.0]

        pol_time_array = [0.0025179386138916016, 0.0032110214233398438, 0.0038940906524658203,
                         0.005976200103759766]
        pol_iters = [3, 4, 4, 2]
        pol_list_scores = [0.954, 0.91, 0.934, 1.0]
        ...
        plt.plot(iters, list_scores, c=cdict['Value Iteration'])
        plt.plot(iters, pol_list_scores, c=cdict['Policy Iteration'])
        plt.xlabel('Iterations')
        plt.ylabel('Avg Utilities')
        plt.title('Frozen Lake'+'Grid Size: ' +str(size) +' - Value vs Policy Iteration - iterations vs utility scores')
        plt.grid()
        plt.legend(cdict)
        plt.savefig('Grid Size: '+str(size)+' Iterations vs utility Analysis' + str('.png'))
        plt.clf()

        #Time vs iterations
        plt.plot(time_array, iters, c=cdict['Value Iteration'])
        plt.plot(time_array, pol_iters, c=cdict['Policy Iteration'])
        plt.xlabel('Time')
        plt.ylabel('Iterations')
        plt.title('Frozen Lake'+'Grid Size: ' +str(size) +' - Value vs Policy Iteration - Time vs Iterations')
        plt.grid()
        plt.legend(cdict)
        plt.savefig('Grid Size: '+str(size)+' Time vs Iterations' + str('.png'))
        plt.clf()
